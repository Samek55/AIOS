...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Steps Today', value: '7,842', goal: '10,000', icon: Footprints, color: '#14B8A6', pct: 78 },
    { label: 'Calories', value: '1,450', goal: '2,000 kcal', icon: Flame, color: '#F97316', pct: 73 },
    { label: 'Tasks Done', value: '5 / 8', goal: '3 remaining', icon: CheckSquare, color: '#2563EB', pct: 63 },
  ...
  return (
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-5">
      ...
      {/* Welcome Banner */}
      <div className="relative rounded-2xl overflow-hidden p-6 flex items-center justify-between shadow-xl shadow-[#17202E]/10">
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg, #17202E 0%, #0F766E 64%, #F97316 150%)' }}
        />
        ...
        <div className="relative z-10">
          <p style={{fontSize:'13px'}} className="text-[#14B8A6]/80 mb-1">{greetingIcon} {greeting},</p>
          <h1 style={{fontSize:'26px', fontWeight:700, lineHeight:'1.2'}} className="text-white">Alex Johnson!</h1>
          <p style={{fontSize:'13px'}} className="text-white/50 mt-1">Sunday, April 5, 2026 · <span className="text-[#14B8A6]">Productivity Score: 78/100</span></p>
          <div className="flex items-center gap-3 mt-4">
            <button
              onClick={() => navigate('/ai-assistant')}
              className="flex items-center gap-2 px-4 py-2 bg-[#14B8A6] text-white rounded-xl hover:bg-[#0F766E] transition-colors"
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span style={{fontSize:'12px'}} className="text-[#14B8A6]">New York, NY</span>
              </div>
            </div>
            <ImageWithFallback src={PROFILE_IMG} alt="Alex" className="w-14 h-14 rounded-full object-cover border-2 border-[#14B8A6]" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white rounded-2xl p-4 border border-[#14B8A6]/20 hover:shadow-md transition-shadow">
            ...
            <p style={{fontSize:'20px', fontWeight:700}} className="text-[#17202E]">{stat.value}</p>
      ...
      {/* AI Insight Banner */}
      <div className="bg-gradient-to-r from-[#17202E] to-[#243244] rounded-2xl p-4 flex items-center gap-4 border border-[#14B8A6]/10">
        <div className="w-10 h-10 rounded-xl bg-[#14B8A6] flex items-center justify-center shrink-0">
          <Brain size={20} className="text-[#17202E]" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span style={{fontSize:'11px', fontWeight:600}} className="text-[#14B8A6] uppercase tracking-wide">AIOS Insight</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            className="px-3 py-1.5 bg-[#14B8A6] text-[#17202E] rounded-lg hover:bg-[#0F766E] transition-colors"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* Today's Schedule */}
        <div className="lg:col-span-1 bg-white rounded-2xl border border-[#14B8A6]/20 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 style={{fontSize:'15px', fontWeight:700}} className="text-[#17202E]">Today's Schedule</h3>
            <button
              onClick={() => navigate('/routine')}
              className="flex items-center gap-1 text-[#0F766E] hover:text-[#17202E] transition-colors"
              style={{fontSize:'12px', fontWeight:600}}
            >
          ...
          <div className="space-y-3">
            {schedule.map((item, i) => (
              <div key={i} className={`flex items-center gap-3 p-2.5 rounded-xl transition-all ${item.done ? 'opacity-50' : 'hover:bg-[#EEF3F8]'}`}>
                <div className="text-center w-14 shrink-0">
                  <p style={{fontSize:'10px'}} className="text-[#888]">{item.time}</p>
                </div>
                <div className={`w-0.5 h-8 rounded-full ${item.done ? 'bg-[#14B8A6]' : 'bg-gray-200'}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span>{item.icon}</span>
                    <p style={{fontSize:'13px', fontWeight: item.done ? 400 : 600}} className={`truncate ${item.done ? 'line-through text-[#aaa]' : 'text-[#17202E]'}`}>
                      {item.title}
                    </p>
                  </div>
                ...
                {!item.done && <div className="w-2 h-2 rounded-full bg-[#14B8A6] shrink-0" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#14B8A6]/20 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 style={{fontSize:'15px', fontWeight:700}} className="text-[#17202E]">Weekly Activity</h3>
            <div className="flex items-center gap-3">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={weeklyActivity} barGap={4}>
              ...
              <YAxis hide />
              <Tooltip
                contentStyle={{fontSize:'12px', borderRadius:'10px', border:'1px solid #E0F7F3', boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}}
                cursor={{fill: '#EEF3F8'}}
              />
              <Bar dataKey="steps" fill="#14B8A6" radius={[5, 5, 0, 0]} maxBarSize={28} />
              <Bar dataKey="calories" fill="#F97316" radius={[5, 5, 0, 0]} maxBarSize={28} />
            </BarChart>
          </ResponsiveContainer>

          {/* Quick Stats below chart */}
          <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-[#14B8A6]/20">
            <div className="text-center">
              <p style={{fontSize:'17px', fontWeight:700}} className="text-[#17202E]">63,882</p>
              <p style={{fontSize:'11px'}} className="text-[#888]">Steps This Week</p>
            </div>
            <div className="text-center border-x border-[#14B8A6]/20">
              <p style={{fontSize:'17px', fontWeight:700}} className="text-[#17202E]">13,050</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#14B8A6]/20 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 style={{fontSize:'15px', fontWeight:700}} className="text-[#17202E]">Quick Order Nearby</h3>
            <button
              onClick={() => navigate('/marketplace')}
              className="flex items-center gap-1 text-[#0F766E] hover:text-[#17202E] transition-colors"
              style={{fontSize:'12px', fontWeight:600}}
          ...
          <div className="mb-4 p-3 bg-[#E0F7F3] rounded-xl border border-[#14B8A6]/30 flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#14B8A6] flex items-center justify-center">
              <Clock size={15} className="text-[#17202E]" />
            </div>
            <div className="flex-1">
              <p style={{fontSize:'13px', fontWeight:600}} className="text-[#17202E]">🍕 Pizza Palace order arriving in 12 min</p>
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-1.5 bg-white rounded-full overflow-hidden">
                  <div className="h-full bg-[#0F766E] rounded-full" style={{width:'70%'}} />
                </div>
                <span style={{fontSize:'11px'}} className="text-[#666]">On the way</span>
              </div>
            </div>
            <button className="text-[#0F766E]" style={{fontSize:'12px', fontWeight:600}}>Track →</button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {quickOrders.map((order) => (
              <div key={order.name} className="flex items-center gap-3 p-3 border border-[#14B8A6]/20 rounded-xl hover:border-[#14B8A6] hover:bg-[#EEF3F8] transition-all cursor-pointer group">
                <ImageWithFallback src={order.img} alt={order.name} className="w-14 h-14 rounded-xl object-cover shrink-0" />
                <div className="flex-1 min-w-0">
                  <p style={{fontSize:'13px', fontWeight:600}} className="text-[#17202E] truncate">{order.name}</p>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span style={{fontSize:'11px', fontWeight:600}} className="text-[#17202E] ml-auto">{order.price}</span>
                  </div>
                </div>
                <button
                  onClick={() => navigate('/marketplace')}
                  className="w-7 h-7 rounded-lg bg-[#17202E] group-hover:bg-[#14B8A6] flex items-center justify-center transition-all shrink-0"
                >
                  <Plus size={13} className="text-white group-hover:text-[#17202E]" />
                </button>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-[#14B8A6]/20 p-5">
            <h3 style={{fontSize:'15px', fontWeight:700}} className="text-[#17202E] mb-1">How are you feeling?</h3>
            ...
            <div className="flex justify-between">
              {moods.map((mood, i) => (
                <button
                  ...
                  className={`text-2xl transition-all hover:scale-125 p-1 rounded-lg ${selectedMood === i ? 'bg-[#E0F7F3] scale-125' : ''}`}
                >
                  {mood}
                </button>
              ))}
            ...
            {selectedMood !== null && (
              <div className="mt-3 p-2.5 bg-[#EEF3F8] rounded-xl">
          ...
          <div className="bg-white rounded-2xl border border-[#14B8A6]/20 p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 style={{fontSize:'15px', fontWeight:700}} className="text-[#17202E]">Smart Alerts</h3>
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={i} className="flex items-start gap-2.5 p-2.5 hover:bg-[#EEF3F8] rounded-xl transition-colors cursor-pointer">
      ...
      <div className="relative rounded-2xl overflow-hidden h-36" style={{background: 'linear-gradient(135deg, #17202E 0%, #3a3a3a 100%)'}}>
        ...
        <div className="relative z-10 p-5 flex items-center justify-between h-full">
          <div>
            <p style={{fontSize:'11px', fontWeight:600}} className="text-[#14B8A6] uppercase tracking-widest mb-1">Today's Challenge</p>
            <h3 style={{fontSize:'20px', fontWeight:700}} className="text-white">Complete your 10k steps</h3>
            <p style={{fontSize:'13px'}} className="text-white/60 mt-1">You need <span className="text-[#14B8A6] font-semibold">2,158 more steps</span> to hit your goal</p>
          </div>
          <button
            onClick={() => navigate('/health')}
            className="px-5 py-2.5 bg-[#14B8A6] text-[#17202E] rounded-xl hover:bg-[#0F766E] transition-colors shrink-0"
            style={{fontSize:'13px', fontWeight:600}}
          >
            View Health
          </button>
